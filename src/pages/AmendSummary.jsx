...
export default function AmendSummary() {
  ...
  const amendSummaryItems = useMemo(() => {
    ...
    state.amendCategoriesUsed.forEach((catKey) => {
      ...
      const { blocks } = computeBlocks(catKey, pbdV, state.amendLineValues, state.amendMatrixValues, state.amendRowsByBlock, state.amendPlakRows, [], noopUpdaters);
      blocks.forEach((blk) => {
    ...
    return items;
  }, [state.amendCategoriesUsed, state.amendPbdVariant, state.amendLineValues, state.amendMatrixValues, state.amendRowsByBlock, state.amendPlakRows]);
